...
global.compose-environment = (pg) ->
  env = {}
  if pg
    env <<< pg{listen, notify, exec, first, relate, related, relation, save, wrap, estrange} <<< pg.model
  for name, lib of olio.lib
    if env[name]
      ...
      if typeof! lib == \Function
        ...
        env[name] = {} <<< lib
    if pg
      env[name] <<< pg{listen, notify, exec, first, relate, estrange, related, relation, save, destroy, wrap}
